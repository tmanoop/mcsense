...
public class TaskSimulation18 extends Simulation {
    ...
    private static final int		N_TASKS_TO_TEST						= 5000;
    //---------------------------------------------------------- SMART POLICIES
    ...
    public TaskSimulation18() throws IOException {
    ...
    class DeadlineCheck extends Event {
    	...
    	public void actions() {
    		theTask.out();
    		if(theTask.global_deadline_in_simulation == (int) time() ){
    			if(theTask.state==QUEUED) System.out.println("Q");
    			if(theTask.state==EXECUTING) System.out.println("E");
    			if(theTask.state==CREATED) System.out.println("C");

    			//new DeadlineCheck(theTask).schedule((int)time()+1);
    		}
	...
	class StopTaskExecution extends Event {
		...
		public void actions() {
			...
			if(probability_success(theTask)){ //true){ //random.draw((double)PERCENTAGE_TASK_FAILURE/100)){ // SUCCESS
				...
				p.money_made += p.min_price[get_price_category(theTask.type)];
