...
public class TaskSimulation22 extends Simulation {
	...
	List<Integer> task_to_match = new ArrayList<Integer>();
	int time_matching=-1;
    ...
    public TaskSimulation22() throws IOException {
    	...
    	public void actions() {
    		if(!over){
	    		if ((int)time() <= nsteps) {
	    			...
	    			//System.out.println("ee "+prov.get_updated_sum_availability());
  ...
  //----------------------------------------------------------------------------------------------------------------------
	...
	public Provider matchTaskWithProvider(Task theTask) throws IOException{
		...
		if(found){ // && time_matching!=(int)time()) {
			task_to_match.add(theTask.identifier);
			if(time_matching!=(int)time()){
				new MatchAtTimeT().schedule((int)time()+OFFSET_MATCHING);
				time_matching=(int)time();
			}
		}
			//matchedProvider = choose_beetwen_subset(theTask, policy_TP);

		return null; //matchedProvider;
	}
	
	Task getTaskFromID(int id){
		Task result = null;
		Task t = (Task) tasklist.first();
		while(t!=null){
			if(t.identifier==id) result = t;
			t = (Task) t.suc();
		}
		if(result==null)
			t = (Task) task_under_completion.first();
		while(t!=null){
			if(t.identifier==id) result = t;
			t = (Task) t.suc();
		}
	...
	class MatchAtTimeT extends Event {
		...
		public void actions() {
			System.out.println(task_to_match.size());
			if(task_to_match.size()==3) {
				System.out.println("wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww");
				// so we actually get more than a task in a second (it was already like that) 
				// probably for the approximation of a double to int
				stopSimulation();
			}

			int task_id = task_to_match.get(0);
			Task t = getTaskFromID(task_id);
	...
	public Provider choose_beetwen_subset(Task theTask, int policy){
		...
		if (policy==FIRST) pp = (Provider) provider_subset.first();
			...
			double min_mm = p.money_made+1;
			int min_ts = p.number_task_received+1;
			double min_m_on_a = p.ipotetical_money/(p.time_has_been_available+1)+1;
			double min_t_on_a = p.number_task_received/(p.time_has_been_available+1)+1;
			double max_global_reliability = p.reliability-1;
			double max_sw_reliability = -1;
			double max_weighted_reliability = -1;


			int diff_ad=0;
			double diff_bp=0;
			double m_on_a_of_i, t_on_a_of_i;
			int reliability_of_i;
			double  reliability_sw_of_i, reliability_weighted_of_i;
			...
			for(int i=0;i<provider_subset.cardinal();i++){
				...
				m_on_a_of_i = p.ipotetical_money/(p.time_has_been_available+1);
				t_on_a_of_i = p.number_task_received/(p.time_has_been_available+1);
				reliability_of_i = p.reliability;
				reliability_sw_of_i = p.get_succ_in_sliding_window();
				reliability_weighted_of_i = reliability_sw_of_i*WEIGHT_SLIDING_WINDOW + reliability_of_i*WEIGHT_GLOBAL_RELIABILITY;
				...
				else if(policy == NORMALIZE_MONEY && p.money_made<min_mm){
					pp=p;
					min_mm=p.money_made;
				}
				else if(policy == NORMALIZE_TASKS && p.number_task_received<min_ts){
					pp=p;
					min_ts=p.number_task_received;
				}
				else if(policy == NORMALIZE_MONEY_ON_AVAILABILITY && m_on_a_of_i<min_m_on_a){
					pp=p;
					min_m_on_a=m_on_a_of_i;
				}
				else if(policy == NORMALIZE_TASKS_ON_AVAILABILITY && t_on_a_of_i<min_t_on_a){
					pp=p;
					min_t_on_a=t_on_a_of_i;
				}
				else if(policy == MAXIMIZE_GLOBAL_RELIABILITY && reliability_of_i>max_global_reliability){ // deprecated
					pp=p;
					max_global_reliability=reliability_of_i;
				}
				else if(policy == MAXIMIZE_SLIDING_WINDOW_RELIABILITY && reliability_sw_of_i>max_sw_reliability){ // deprecated
					pp=p;
					max_sw_reliability=reliability_sw_of_i;
				} 
				else if(policy == MAXIMIZE_WEIGHTED_RELIABILITY && reliability_weighted_of_i>max_weighted_reliability){
					pp=p;
					max_weighted_reliability=reliability_weighted_of_i;
				}
