...
public class TaskSimulation6 extends Simulation {
    ...
    public TaskSimulation6() throws IOException {
		...
		FileInputStream fstream = new FileInputStream("rwm.txt");
		...
		int nusers=Integer.parseInt(st.nextToken()); //1000;
		int nsteps= Integer.parseInt(st.nextToken())+1; //601;
		System.out.println(nusers+" - "+nsteps);
		//Array[] mobtraces = new Array[100];
		Row[][] mobtraces = new Row[nusers][nsteps];
		int user;
		int temp_time;
		double temp_x;
		double temp_y;
		...
		//for(int j=0;j<nminutes;j++){
		
		logger.info("Loading Mobility Traces in memory from "+nusers+" users for "+nsteps+" unit steps");
		...
		//System.out.println(2+" "+mobtraces[2][3].time+" "+mobtraces[2][3].x+" "+mobtraces[2][3].y);
		
		System.out.println("Printing test!");
		for(int i=0;i<nusers;i++){
			for(int j=0;j<nsteps;j++){
				Row r= (Row) mobtraces[i][j];
				System.out.println(i+"-"+r.time+"-"+r.x+"-"+r.y);
			}
		}
    ...
    class ProviderArrival extends Event {
    	...
    	public void actions() {
    		if (time() <= SIM_PERIOD) {
    	    	...
    	    	//available=true;
    	    	int id_provider_who_sent_notification = random.randInt(0, NUMBER_OF_REGISTRED_PROVIDERS-1);
