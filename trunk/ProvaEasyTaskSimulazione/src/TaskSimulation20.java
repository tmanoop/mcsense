...
public class TaskSimulation20 extends Simulation {
    ...
    private static final int		N_TASKS_TO_TEST						= 10;
    //---------------------------------------------------------- SMART POLICIES
    ...
    int policy = NORMALIZE_TASKS_ON_AVAILABILITY; //MIN_BUDGET_PRICE; //FIRST; //NORMALIZE_MONEY_ON_AVAILABILITY; //MIN_AVAILABILITY_DEADLINE; //FIRST; //NORMALIZE_MONEY; //NORMALIZE_TASKS; //FIRST; //MIN_BUDGET_PRICE; //MIN_AVAILABILITY_DEADLINE; //LAST; //FIRST;
    ...
    public TaskSimulation20() throws IOException {
    ...
    class ProviderArrival extends Event {
    	...
    	public void actions() {
    		if (time() <= nsteps) {
    			...
    			//prov.get_updated_sum_availability(); // uu
    	    	//prov.update_availability();
    	    	//if(prov.is_available()) prov.keep_updating=true; //uu
		...
		for(int i=0;i<nusers;i++){
			...
			if(	( dont_consider_price 		 || 	array_provider[i].min_price[get_price_category(theTask.type)]
				...
				(dont_consider_hard_availability || time()+theTask.duration <=  array_provider[i].available_up_to)
					...
					System.out.println("uu" +matchedProvider.get_updated_sum_availability()); //update only of the possible candidates
					//break;	
				}
				if(steps==1){
