...
public class HardwareMonitoringAlarm extends BroadcastReceiver {

	private final String REMINDER_BUNDLE = "HardwareMonitoringReminderBundle";
	private JTask currentTask;
	private long serverTime;
	
	// this constructor is called by the alarm manager.
	public HardwareMonitoringAlarm() {
	}
	...
	// and the timeout
	public HardwareMonitoringAlarm(Context context, Bundle extras, int timeoutInSeconds) {
		...
		Intent intent = new Intent(context, HardwareMonitoringAlarm.class);
		...
		Toast.makeText(context, "WiFi Sensing Started", Toast.LENGTH_SHORT).show();
...
//		Toast.makeText(context, "Bluetooth Scan started", Toast.LENGTH_SHORT).show();
		...
		if(!hasTaskExpired(context)){
			WifiScanCount++;
			logWifiScanCount(WifiScanCount, context);
			iniHardwareMonitorService(context);		
		} else {
			AppUtils.uploadHardwareSensedData(context, currentTask);
		}		
	}

	protected void iniHardwareMonitorService(Context context) {
	...
	protected void logWifiScanCount(int WifiScanCount, Context context) {
